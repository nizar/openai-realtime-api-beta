...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  constructor({
    ...
    this.debug = !!debug;
    this.model = model || 'gpt-4o-realtime-preview-2024-12-17';
    this.ws = null;
   ...
   */
  isAzureOpenai() {
    return this.url && this.url.indexOf('.azure.com') !== -1;
  }
  ...
  async connect({ model } = {}) {
    ...
    if (globalThis.WebSocket) {
      ...
      const ws = this.isAzureOpenai()
        ? new WebSocket(
            `${this.url.replace('https://', 'wss://')}${model ? `&api-key=${this.apiKey}` : ''}`,
          )
        : new WebSocket(`${this.url}${model ? `?model=${model}` : ''}`, [
            'realtime',
